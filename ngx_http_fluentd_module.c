...
// This function copied from https://github.com/zebrafishlabs/nginx-statsd/pull/30/files
//ngx_udp_connect()
static ngx_int_t
ngx_http_fluentd_udp_connect(ngx_resolver_connection_t *rec)
{
    int                rc;
    ngx_int_t          event;
    ngx_event_t       *rev, *wev;
...
ngx_http_fluentd_send(ngx_udp_endpoint_t *l, u_char *buf, size_t len)
{
    ...
    if (uc->udp == NULL) {
        if(ngx_http_fluentd_udp_connect(uc) != NGX_OK) {
            if(uc->udp != NULL) {
